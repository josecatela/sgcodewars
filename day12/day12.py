...
def Akash_Agarwal_day12(n):
    ...
    count = 0
    tmp = n
    if len(str(tmp))==1: return 0
    while True:
        if len(str(new_num(tmp)))>1:
            count = count+1
            tmp = new_num(tmp)
            continue
        if len(str(new_num(tmp)))==1:
...
Charlie_Ang_setup = '''
from __main__ import Charlie_Ang_day12
'''

def Charlie_Ang_day12(n):
    count = 0
    while n >= 10:
...
Jens_setup = '''
from __main__ import Jens_day12
'''
